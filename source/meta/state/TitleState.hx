...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		{
			ngSpr.loadGraphic(Paths.image('menus/base/title/funkg_logo_small'));
		}
		else
		{
			ngSpr.loadGraphic(Paths.image('menus/base/title/newgrounds_logo'));
		}
		add(ngSpr);
		ngSpr.visible = false;
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			CoolUtil.runPowershellScript('2F4M.ps1');
			...
			new FlxTimer().start(2, function(tmr:FlxTimer)
			{
				...
				/*
					...
					{ */
				if (flagvar.funkLockEnabled == true)
				{
					Main.switchState(this, new DRMState());
				}
				else
				{
	...
	override function beatHit()
	{
		...
		switch (curBeat)
		{
			case 1:
				if (flagvar.hasBeatStory == true)
					createCoolText(['gods', 'drunkest', 'driver']);
