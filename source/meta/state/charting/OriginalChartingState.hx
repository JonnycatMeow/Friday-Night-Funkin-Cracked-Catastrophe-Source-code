...
class OriginalChartingState extends MusicBeatState
{
	...
	**/
	...
	var vocals:FlxSound;
	var vocalsOpp:FlxSound;
	var hasSplitVocals:Bool = false;
	...
	function loadSong(daSong:String):Void
	{
		...
		//check for split vocals
		hasSplitVocals = Paths.doSplitVocalsExist(daSong);

		if (_song.needsVoices && hasSplitVocals)
		{
			vocals = new FlxSound().loadEmbedded(Paths.voices(daSong), false, true);
			vocalsOpp = new FlxSound().loadEmbedded(Paths.voicesOpp(daSong), false, true);
		}

		else if (_song.needsVoices && !hasSplitVocals)
		{
			vocals = new FlxSound().loadEmbedded(Paths.voices(daSong), false, true);
