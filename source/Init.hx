...
class Init extends FlxState
{
	...
	public static var gameSettings:Map<String, Dynamic> = [
		...
		'Streamer Mode' => [
			false,
			Checkmark,
			"Switches out some potentially personally-identifying info so people don't stalk you.",
			NOT_FORCED
		],
		'Shaders' => [
			true,
			Checkmark,
			"Enables shaders. NOTE: This should only be disabled if you're crashing.",
			NOT_FORCED
		],
		'Raid Chat' => [
			true,
			Checkmark,
			"Toggles the visibility of the chat in boss raids.",
			NOT_FORCED
		]
	...
	override public function create():Void
	{
		...
		//coconut lol
		if(!FileSystem.exists('assets/images/coconut.jpg'))
			throw "Coconut not found!";
		/*{
			Sys.command('msg %username% Coconut.jpg not found.');
			Sys.exit(1);
		}*/

		//throw in a quick raid check for any 1.0 chads, will be removed in 1.2 if I decide to make it
