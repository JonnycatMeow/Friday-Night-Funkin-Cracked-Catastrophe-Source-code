...
class DialogueBox extends FlxSpriteGroup
{
	...
	public var username:String = FlxG.save.data.username;

	public static var unparsedJson:String;

	public static var replacedStrings:Int = 0;

	public var unskippable:Bool = false;

	public var hasSteam:Bool = false;

	public var installedGames:Array<Bool> = []; //order is very important here, it goes hacknet, spacewar, among us, average4k, and finally madness
	...
	public function dialoguePath(file:String):String
	{
		...
		{
			return dialoguePath;
			trace("found dialogue path: " + dialoguePath);
		}
		else
		{
			return truePath;
			trace("couldn't find dialogue path, using true path: " + truePath);
		}
	}
	...
	public function new(?talkingRight:Bool = false, ?daDialogue:String)
	{
		...
		trace("start");
				...
				//whoops, gotta pop in real quick for some fancy MULTIPLE dialogue files hehe
		if (PlayState.SONG.song.toLowerCase() == 'piracy-check')
		{
			if (Init.trueSettings.get("Streamer Mode") == true)
			{
				//daDialogue = daDialogue + username;
				//trace("streamer mode is apparently on, lookin for " + daDialogue);
			}
			else if (Init.trueSettings.get("Streamer Mode") == false)
			{
				//trace("streamer mode is apparently off, lookin for " + daDialogue);
			}
		}

		//grab the unparsed json for the data dump in PlayState
		unparsedJson = daDialogue;

		// get dialog data from dialogue.json
		dialogueData = haxe.Json.parse(daDialogue);

		//sorry shubs, gotta change this up a lil to support some username shit
		if (PlayState.SONG.song.toLowerCase() == 'piracy-check' || PlayState.SONG.song.toLowerCase() == 'overflow' || PlayState.SONG.song.toLowerCase() == 'dbdos')
		{
				// switch out any instance of the string [USERNAME] in DialogueFileDataDef's Dialog array with the username var
				//please god let these things be strings
				for (i in dialogueData.dialogue)
				{
					if(i.text.toLowerCase().contains("[username]"))
					{
						var oldText = i.text;
						i.text = i.text.replace("[username]", username);
						trace("replaced a username: " + oldText + " >>> " + i.text);
						replacedStrings++;
					}
				}
	...
	public function closeDialogInstant()
	{
		alphabetText.playSounds = false;
		kill();
	}
	...
	public function checkForSteam()
	{
		var hasSteamInstalled:Int = Sys.command("REG QUERY HKCU\\SOFTWARE\\Valve\\Steam");
		if (hasSteamInstalled == 0)
		{
			trace("Steam is installed");
			hasSteam = true;
			return true;
		}
		else
		{
			trace("Steam is not installed");
			hasSteam = false;
			return false;
		}
	}
	...
	public function checkSteamGames()
	{
		...
		var hasHacknet = Sys.command("REG QUERY HKCU\\SOFTWARE\\Valve\\Steam\\Apps\\365450");
		var hasSpacewar = Sys.command("REG QUERY HKCU\\SOFTWARE\\Valve\\Steam\\Apps\\480");
		var hasAmongUs = Sys.command("REG QUERY HKCU\\SOFTWARE\\Valve\\Steam\\Apps\\945360");
		var hasAverage4k = Sys.command("REG QUERY HKCU\\SOFTWARE\\Valve\\Steam\\Apps\\1828580");
		var hasMadness = Sys.command("REG QUERY HKCU\\SOFTWARE\\Valve\\Steam\\Apps\\488860");

		if (hasHacknet == 0)
		{
			installedGames.push(true);
			trace("player has hacknet");
		}
		else
			installedGames.push(false);

		if (hasSpacewar == 0)
		{
			installedGames.push(true);
			trace("player has spacewar");
		}
		else
			installedGames.push(false);
		...
		{
			installedGames.push(true);
			trace("player has madness");
		}
		else
			installedGames.push(false);

		trace("done checking for games");
		trace("here's the array:");
	...
	{
		var garbage; //used to hold removed dialogue since haxe won't let me remove elements by index REEEEEEEEEEEE
		var hasGoodTaste = false;
		if (hasSteam == false)
		{
			garbage = dialogueData.dialogue.splice(5, 8);
			//trace all the removed dialogue
			for (i in garbage)
			{
				trace(i);
			}
		...
		else
		{
			...
			//i'm not sorry
			//we work backwards so we don't fuck up the index

			//first let's check if the player has any of the games
			//if they don't, we'll nuke the dialogue for the games and keep in the 'your taste in games is shit' line

			//let's start with getting rid of the 'no? shame' line
			garbage = dialogueData.dialogue.splice(13, 1);
			garbage = null;
			...
			if (hasGoodTaste == true)
			{
				//let's nip that 'shit taste' line in the bud
				garbage = dialogueData.dialogue.splice(12, 1);
				garbage = null;
			}

			if (hasGoodTaste == false)
			{
				//someone's got shit taste WEEWOO WEEWOO
				garbage = dialogueData.dialogue.splice(7, 5);
				//trace all the removed dialogue for debugging
				for (i in garbage)
				{
					trace(i);
				}
				//then take out the trash!
				garbage = null;
				//then we'll get out of this function as we don't need to check for games
				return;
			}

			if (installedGames[4] == false)
			{
				garbage = dialogueData.dialogue.splice(11, 1);
				trace("removed madness dialogue");
				garbage = null;
			}

			if (installedGames[3] == false)
			{
				garbage = dialogueData.dialogue.splice(10, 1);
				trace("removed a4k dialogue");
				garbage = null;
			}

			if (installedGames[2] == false)
			{
				garbage = dialogueData.dialogue.splice(9, 1);
				trace("removed among us dialogue");
				garbage = null;
			}

			if (installedGames[1] == false)
			{
				garbage = dialogueData.dialogue.splice(8, 1);
				trace("removed spacewar dialogue");
				garbage = null;
			}

			if (installedGames[0] == false)
			{
				garbage = dialogueData.dialogue.splice(7, 1);
				trace("removed hacknet dialogue");
				garbage = null;
			}
			...
			//then we'll get out of this function as we don't need to check for anything else
			return;
