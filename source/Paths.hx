...
class Paths
{
	...
	public static function returnSound(path:String, key:String, ?library:String) {
		...
		trace('returnSound path:' + gottenPath);
	...
	inline static public function voices(song:String, ?freeplayChar:String = ""):Any {
		var songKey:String;
		if (song.contains(" "))
			song.replace(" ", "-");
		if (freeplayChar != "")
		{
			trace('looking for voices for $freeplayChar in $song');
			if (doCharVocalsExist(song, freeplayChar))
				songKey = song + "/" + freeplayChar.toLowerCase();
			else
				songKey = song + "/bf";
			//songKey = '${CoolUtil.spaceToDash(song.toLowerCase())}/${freeplayChar.toLowerCase()}';
		}
		else
		{
			songKey = '${CoolUtil.swapSpaceDash(song.toLowerCase())}/bf';
		}
		var voices = returnSound('songs', songKey);
		trace('returning player vocal path: $songKey');
		return voices;
	}

	inline static public function voicesOpp(song:String):Any {
	...
	inline static public function doSplitVocalsExist(song:String)
	{
	...
	inline static public function doCharVocalsExist(song:String, char:String)
	{
		var hasVocals:Bool = false;
		song = 'assets/songs/${CoolUtil.spaceToDash(song.toLowerCase())}/${char.toLowerCase()}.ogg';
		if (FileSystem.exists(song))
		{
			trace('unique vocals found for $char in $song');
			hasVocals = true;
		}
		else
		{
			trace('no unique vocals found for $char in $song');
			hasVocals = false;
		}
		return hasVocals;
	}

	inline static public function doesSaveDataExist(mod:String)
	{
		//a little helper function to check if save files exist for specific mods

		//TODO:
		//port over air marshal sprite
		//FUCKING FIX OVERFLOW'S SCROLLSPEED
		//maybe ask funkg for character suggestions? max 5 probably

		//let's start off by making a path directly to the appdata folder
		var path:String = Sys.getEnv("APPDATA");
		switch(mod)
		{
			case "vtan":
				path = path + "\\kadedev\\Vs V\\ninjamuffin99\\funkin.sol";
				trace("looking for V-tan save data in: " + path);
				if(FileSystem.exists(path))
				{
					trace("save data found for Vs V!");
					return true;
				}
				else
				{
					trace("no save data found for Vs V :(");
					return false;
				}
			
			case "airmarshal":
				//TSURARAN YOU FUCKING BASTARD THERE'S NO FUCKING SAVE DATA FOR AIR MARSHAL BECAUSE IT FUCKING CRASHES ITSELF UPON FINISHING THE SONG
				//THAT MEANS THAT IT CAN'T SAVE THE SCORE YOU SHITLORD
				//so instead, we look for salty!
				path = path + "\\ninjamuffin99\\Salty\\ninjamuffin99\\funkin.sol";
				trace('looking for SSN save data in: ' + path);
				if(FileSystem.exists(path))
				{
					trace("save data found for salty's sunday night!");
					return true;
				}
				else
				{
					trace("no save data found for salty's sunday night :(");
					return false;
				}

			case "afton":
				path = path + "\\ShadowMario\\FunkinAtFreddys\\ninjamuffin99\\funkin.sol";
				trace("looking for FAF save data in: " + path);
				if(FileSystem.exists(path))
				{
					trace("save data found for funkin at freddy's!");
					return true;
				}
				else
				{
					trace("no save data found for funkin at freddy's :(");
					return false;
				}

			case "mario":
				path = path + "\\ninjamuffin99\\Funk Mix\\foreverengine-options.sol"; //weird place to have all the save data, but whatever.
				trace("looking for funk mix save data in: " + path);
				if(FileSystem.exists(path))
				{
					trace("save data found for funk mix!");
					return true;
				}
				else
				{
					trace("no save data found for funk mix :(");
					return false;
				}

			default:
				trace("could not find save data for $mod - invalid name?");
				return false;
		}
	}
}
